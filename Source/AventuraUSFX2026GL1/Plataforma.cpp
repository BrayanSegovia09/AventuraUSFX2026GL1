...
APlataforma::APlataforma()
{
	...
	*/
	...
	//Colision_Estructura = CreateDefaultSubobject<UBoxComponent>(TEXT("Colision")); // Crear
	//Colision_Estructura->SetupAttachment(RootComponent); // Poner raiz

	//Colision_Estructura->SetBoxExtent(FVector(50.0f, 50.0f, 50.0f)); // tamano del box
...
void APlataforma::Tick(float DeltaTime)
{
	...
	*/
	...
	if (TiempoCiclo >= 5.0f)
	{
		...
		if ((posicionActual.X >= movimientoLimitesMaximos.X && movimientoDireccion.X >= 1) || 
			(posicionActual.X <= movimientoLimitesMinimos.X && movimientoDireccion.X <= -1)) {
			movimientoDireccion.X *= -1.0f;
		}

		if ((posicionActual.Y >= movimientoLimitesMaximos.Y && movimientoDireccion.Y >= 1) ||
			(posicionActual.Y <= movimientoLimitesMinimos.Y && movimientoDireccion.Y <= -1)) {
			movimientoDireccion.Y *= -1.0f;
		}

		if ((posicionActual.Z >= movimientoLimitesMaximos.Z && movimientoDireccion.Z >= 1) ||
			(posicionActual.Z <= movimientoLimitesMinimos.Z && movimientoDireccion.Z <= -1)) {
...
//void APlataforma::NotifyActorBeginOverlap(AActor* OtherActor) {
//	Super::NotifyActorBeginOverlap(OtherActor);
//
//	if (OtherActor->IsA(AAventuraUSFX2026GL1Pawn::StaticClass())) {
//		AAventuraUSFX2026GL1Pawn* nave = Cast<AAventuraUSFX2026GL1Pawn>(OtherActor);
//
//		/*if (nave) {
//
//		}*/
